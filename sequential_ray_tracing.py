...
# Convenience functions for keeping track of tunable parameters

class Parameters(collections.abc.Mapping):
    """A convenience class for keeping track of the tunable parameters
    of an OpticalSystem.

    If we do it right, this should feel like the easiest way to specify
    parameters, initial values, and (optionally), optimization options like
    learning rates.
    """
    @classmethod
    def from_fn(cls, fn):
        """ The annotation of each parameter of the function `fn` is expected 
        to be a dict of per-parameter options for torch.optim.
        """
        sig = inspect.signature(fn)
        kwargs = {}
        for k, v in sig.parameters.items():
            annotation = v.annotation if v.annotation != inspect._empty else {}
            assert v.default != inspect._empty, (
                "All parameters must have a default value.")
            assert isinstance(annotation, dict)
            kwargs[k] = (v.default, annotation)
        return cls(**kwargs)
    ...
    def __init__(self, **kwargs):
        ...
        for k, v in kwargs.items():
            # Convert the parameter to a tensor with requires_grad=True
            try: # Just a bare number?
                initial_value = float(v)
            except TypeError: 
                # If the value has a second entry, it's a dict of per-parameter
                # options for torch.optim
                assert len(v) == 2
                initial_value = float(v[0])
                self.optimization_options[k] = dict(v[1])
            tensor_value = npt.as_tensor(initial_value)
            if torch: tensor_value.requires_grad = True
            self.tensors[k] = tensor_value
        return None

    def to_values(self):
        return {k: float(npt.asarray(v)) for k, v in self.tensors.items()}

    def to_optim(self):
        """e.g. optim = torch.optim.Adam(params.to_optim(), lr=1e-3)"""
        return [{'params': [self.tensors[k]],
                 **self.optimization_options.get(k, {})} for k in self.tensors]

    def set_metadata(self, **kwargs): # This barely needs to exist
        self.metadata.update(kwargs)

    def append_to_file(self, file):
        # It's a lot cheaper to store a float on disk than a tensor...
        stub = Parameters()
        stub.tensors = self.to_values()
        stub.optimization_options = self.optimization_options
        stub.metadata = self.metadata
        with open(file, 'ab') as fout:
            pickle.dump(stub, fout)
    ...
    def __getitem__(self, index):
        return self.tensors.__getitem__(index)
        ...
        with open(file, 'rb') as fin:
            while True:
                try:
                    ...
                    # Hard to tell what kind of error pickle is going to throw
                    # if it encounters a halfwritten record.
                    print("Found a corrupted pickle of a parameter...")
                    print(e)
                    break
                params.tensors = {
                    k: npt.as_tensor(v) for k, v in params.tensors.items()}
                for v in params.tensors.values():
                    v.requires_grad = True
                sequence.append(params)
        return cls(sequence)

    def __init__(self, sequence_of_parameters):
        assert len(sequence_of_parameters) > 0
        assert all(isinstance(p, Parameters) for p in sequence_of_parameters)
        keys = sequence_of_parameters[0].tensors.keys()
        md_keys = sequence_of_parameters[0].metadata.keys()
        assert all(p.tensors.keys()  == keys    for p in sequence_of_parameters)
        assert all(p.metadata.keys() == md_keys for p in sequence_of_parameters)

        self.sequence = sequence_of_parameters
        self.value_histories = {k:[] for k in keys}
        self.metadata_histories = {k:[] for k in md_keys}
        for p in self.sequence:
            for k, v in p.to_values().items():
                self.value_histories[k].append(v)
            for k, v in p.metadata.items():
                self.metadata_histories[k].append(v)
        for k in keys:
            self.value_histories[k] = np.array(self.value_histories[k])
        for k in md_keys:
            self.metadata_histories[k] = np.array(self.metadata_histories[k])

    def __getitem__(self, index):        
        if isinstance(index, str):
            if index in self.value_histories:
                return self.value_histories[index]
            elif index in self.metadata_histories:
                return self.metadata_histories[index]
            else:
                raise KeyError(index)
        elif np.issubdtype(int, np.integer) or isinstance(index, slice):
            return self.sequence.__getitem__(index)
        else:
            raise TypeError("Index must be str, int, or slice")
